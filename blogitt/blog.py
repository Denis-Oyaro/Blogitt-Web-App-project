...
from blogitt.auth import login_required
...
def index():
    db = get_db()
    posts = db.execute('SELECT p.id, title, body, created, author_id, username FROM'
               ' post p JOIN user u ON p.author_id = u.id ORDER BY created DESC').fetchall()
